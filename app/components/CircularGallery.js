...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Set Y to 95% of viewport height since we removed the text and have a thin container
    this.plane.scale.y = this.viewport.height * 0.95;
    
    // Calculate X scale to make the cards proportionally taller (1.4x height instead of 2.5x)
    const targetPhysicalWidth = this.screen.height * 0.95 * 1.4;
    this.plane.scale.x = (targetPhysicalWidth / this.screen.width) * this.viewport.width;
...
class App {
  ...
  onTouchUp(e) {
    ...
    if (e && this.onItemClick) {
      ...
      if (Math.abs(this.start - end) < 5 && Math.abs(this.startY - endY) < 5) {
        const rect = this.container.getBoundingClientRect();
        const x = ((end - rect.left) / rect.width) * 2 - 1;
        ...
        if (hits.length > 0) {
          ...
          if (clickedMedia && clickedMedia.link) {
            this.onItemClick(clickedMedia.link);
          }
        }
      }
    }
  ...
  onWheel(e) {
    ...
    // Multiply by the actual delta value to make scrolling velocity perfectly smooth and proportional to the user's trackpad swipe
    this.scroll.target += delta * 0.03 * this.scrollSpeed;
  ...
  onResize() {
    ...
    // Center the scene perfectly vertically
...
export default function CircularGallery({
  ...
  return (
    <div
      ...
      className={`w-full h-full overflow-hidden cursor-grab active:cursor-grabbing focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-4 ${className}`}
    />
  );
}
